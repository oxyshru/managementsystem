...
interface PlayerSeed extends Omit<Player, 'id' | 'userId' | 'stats' | 'createdAt' | 'updatedAt' | 'attendance' | 'lastAttendance' | 'batch'> { userId: number }
...
interface PaymentSeed extends Omit<Payment, 'id' | 'createdAt' | 'updatedAt'> {}
interface PerformanceNoteSeed extends Omit<PerformanceNote, 'id' | 'createdAt' | 'updatedAt'> {} // Use PerformanceNote
interface PlayerGameSeed { player_id: number; game_id: number; }
interface TrainingSessionSeed extends Omit<TrainingSession, 'id' | 'createdAt' | 'updatedAt'> {}
...
const initialSeedData: {
    ...
    players: [
        // Remove the mock IDs here, as they will be generated by the DB
    ...
    batches: [
        // Remove the mock IDs here, link via game name and coach name/specialization if needed for lookup
     ...
     training_sessions: [
         // Remove mock IDs, link via batch name/game if needed for lookup
    ...
    payments: [
        // Remove mock IDs, link via player name/email if needed for lookup
    ...
    performance_notes: [
        // Remove mock IDs, link via player/coach if needed for lookup
...
(4, 2, NOW(), NOW()); -- Sarah Davis (player 4) -> Swimming (game 2) -- Removed extra NOW()
